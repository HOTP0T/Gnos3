<script lang="ts">
	...
	// Finances (ledger-sync) sidebar entry: only when the dispatcher reports the
	// module enabled AND the user has read access (admins bypass).
	$: financesEnabled = _modulesNow.some((m) => m.name === 'finances');
	$: financesVisible =
		financesEnabled && ($isAdmin || Boolean($user?.permissions?.modules?.finances?.read));
					...
					<div id="pinned-menu-items-list">
						...
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							<a
								...
								on:click={itemClickHandler}
								draggable="false"
								aria-label={$i18n.t('K4mi Documents')}
							>
								<div class="self-center">
									<svg
										xmlns="http://www.w3.org/2000/svg"
								...
								<div class="flex self-center translate-y-[0.5px]">
									<div class="self-center text-sm font-primary">{$i18n.t('K4mi Documents')}</div>
								</div>
							</a>
						</div>
						...
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							...
							<!-- Finances (visible when the finances module is enabled + read perm) -->
							{#if financesVisible}
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							<a
								id="sidebar-finances-button"
								class="grow flex items-center space-x-3 rounded-2xl px-2.5 py-2 hover:bg-gray-100 dark:hover:bg-gray-900 transition"
								href="/finances"
								on:click={itemClickHandler}
								draggable="false"
								aria-label={$i18n.t('Finances')}
							>
								<div class="self-center">
									<svg
										xmlns="http://www.w3.org/2000/svg"
								...
								<div class="flex self-center translate-y-[0.5px]">
									<div class="self-center text-sm font-primary">{$i18n.t('Finances')}</div>
								</div>
							</a>
						</div>
							{/if}
