<script lang="ts">
	...
	onMount(async () => {
		const canAccess =
			$isAdmin ||
			Boolean($user?.permissions?.modules?.accounting?.read);
		if (!canAccess) {
			await goto('/');
			return;
		}
...
<svelte:head>
	<title>
		{$i18n.t('Accounting')} &bull; {$WEBUI_NAME}
	</title>
</svelte:head>
		...
		<nav class="px-2.5 pt-1.5 backdrop-blur-xl drag-region">
			<div class="flex items-center gap-1">
				...
				<div class="">
					...
					>
						<a
							class="min-w-fit p-1.5 {$page.url.pathname.includes('/accounting/compan')
								? ''
								: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition"
							href="/accounting/companies">{$i18n.t('Companies')}</a
						>
						<a
							class="min-w-fit p-1.5 {$page.url.pathname === '/accounting/settings'
								? ''
								: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition"
							href="/accounting/settings">{$i18n.t('Settings')}</a
						>
					</div>
				</div>
			</div>
