		...
		<nav class="px-2.5 pt-1.5 backdrop-blur-xl drag-region">
			<div class="flex items-center gap-1">
				...
				<div class="">
					...
					>
						<a
							class="min-w-fit p-1.5 {$page.url.pathname.includes('/invoices/dashboard')
								? ''
								: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition"
							href="/invoices/dashboard">{$i18n.t('Dashboard')}</a
						>
						<a
							class="min-w-fit p-1.5 {$page.url.pathname.includes('/invoices/data')
								? ''
								: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition"
							href="/invoices/data">{$i18n.t('Data')}</a
						>
					</div>
				</div>
			</div>
